...
void ctrl_input(const geometry_msgs::Vector3::ConstPtr& inputs){
...
int main(int argc, char **argv){
    ...
    //Node Set up
    ros::init(argc, argv, "dlm_dyn_sim");
    ros::NodeHandle nh;

    //Publisher set up
    ros::Publisher joints_pub = nh.advertise<sensor_msgs::JointState>("/joint_states",10);
    ros::Publisher joints_feedback_pub = nh.advertise<geometry_msgs::Quaternion>("/joint_feedback",10);

    //Subscriber set up
    ros::Subscriber ctr_sub =  nh.subscribe<geometry_msgs::Vector3>("/ctrl_dlm",10,&ctrl_input);
    ...
    geometry_msgs::Quaternion joints_states_fb;
    ...
    while (ros::ok()){
        ...
        joints_states_fb.x = q(0);
        joints_states_fb.y = q(1);
        joints_states_fb.z = q_dot(0);
        joints_states_fb.w = q_dot(1);

        joints_feedback_pub.publish(joints_states_fb);
